...
contract TokenPresale is Ownable {
    ...
    function depositByBNB(uint256 bnbprice) payable public {
        require(msg.value > minInvestment, "");

        uint256 bnbvalue = msg.value;
        uint256 bnbrate = 676*(10**14);
        uint256 _tokenamount = bnbvalue.mul(bnbprice).div(bnbrate,"");
        FeelCoin(feelcoinAddress).transfer(msg.sender, _tokenamount);
        payable(withdrawAddress).transfer(address(this).balance);

    }
    
    function depositByFiat(uint _amount,address reciever) public {
        require(_amount > minInvestment, "");
        require(validateDeposit(_amount));
